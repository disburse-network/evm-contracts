...
import { keccak256, JsonRpcProvider, ContractFactory, parseEther, parseUnits, MaxUint256, computeAddress, Interface, id, Wallet as SignerWallet, Contract } from 'ethers';
...
function createManualTimelocks(deployedAt: number): bigint {
  ...
  const srcPublicWithdrawal = BigInt(120); // 2 minutes
...
const EVM_PRIVATE_KEY = process.env.OPTIMISM_PRIVATE_KEY || '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80';
  ...
  try {
    ...
    if (txnResponse.ok) {
      ...
      if (fusionOrderAcceptedEvent && escrowCreatedEvent) {
        ...
        console.log(`   Destination Asset: 0x${Buffer.from(fusionOrderAcceptedEvent.destination_asset.slice(2), 'hex').toString('hex')}`);
        console.log(`   Destination Recipient: 0x${Buffer.from(fusionOrderAcceptedEvent.destination_recipient.slice(2), 'hex').toString('hex')}`);
...
async function createFusionOrder() {
  ...
  try {
    ...
    console.log(`   Initial destination amount: ${initialDestinationAmount} (${initialDestinationAmount / 1e6} USDC)`);
...
async function deployDst(escrowEventData: EventData): Promise<any> {
  ...
  const approve = erc20.interface.encodeFunctionData('approve', [src.escrowFactory, parseEther("1")]);
  ...
  const escrowFactoryContract = new Contract(src.escrowFactory, factoryContract.abi, wallet);
  
  // Verify the contract is connected to a signer
  if (!escrowFactoryContract.runner) {
  ...
  const immutables = [
    ...
    escrowEventData.fusionOrder.resolver, // taker // Address
  ...
  console.log('  - Contract runner type:', escrowFactoryContract.runner?.constructor.name);
  
  try {
    // Try using the contract interface directly
    const iface = new Interface(factoryContract.abi);
    const data = iface.encodeFunctionData('createDstEscrow', [immutables, cancellationTimestamp]);
    
    console.log('Encoded function data:', data);
    console.log('Function signature:', iface.getFunction('createDstEscrow')?.format());
    
    const tx = await wallet.send({
      to: src.escrowFactory,
      data: data,
      value: nativeAmount,
      from: await wallet.getAddress()
    });
    ...
    return {
      txHash: receipt,
      escrowAddress: 'mock_escrow_address' // In production, you'd extract this from events
    };
  } catch (error) {
...
async function main() {
  try {
    ...
    const evmResult = await deployDst(escrowEventData);
    ...
    console.log(`   💰 Source Amount: ${escrowEventData.fusionOrder.sourceAmount} octas (${escrowEventData.fusionOrder.sourceAmount / 1e8} APT)`);
    console.log(`   🎯 Destination Asset: 0x${Buffer.from(escrowEventData.fusionOrder.destinationAsset.slice(2), 'hex').toString('hex')} (USDC)`);
    console.log(`   👥 Destination Recipient: 0x${Buffer.from(escrowEventData.fusionOrder.destinationRecipient.slice(2), 'hex').toString('hex')}`);
    console.log(`   💎 Accepted Price: ${escrowEventData.fusionOrder.currentPrice} (${escrowEventData.fusionOrder.currentPrice / 1e6} USDC)`);
    
    console.log("\n🔐 ATOMIC SWAP SECRETS:");
    console.log(`   🔑 Secret (for withdrawal): ${orderResult.secret}`);
    console.log(`   🔒 Hash (in contracts): ${escrowEventData.fusionOrder.hash}`);
    console.log(`   ✅ Hash Verification: ${orderResult.hashHex === escrowEventData.fusionOrder.hash ? 'MATCH ✅' : 'MISMATCH ❌'}`);
    ...
    console.log("\n🔗 EXPLORER LINKS:");
    console.log(`   📱 Aptos Order Creation: https://explorer.aptoslabs.com/txn/${orderResult.txHash}?network=testnet`);
